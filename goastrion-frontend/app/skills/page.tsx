...
function PlanetAvatar({ name }: { name: string }) {
  const glyph = PLANET_EMOJI[name] ?? name[0];
  return (
    <span
      title={name}
...
function ScoreRing({ value }: { value: number }) {
  ...
  // conic ring using inline style (safe here)
  const circle = {
...
export default function SkillsPage() {
  ...
  // Search + sort + topN slice
  const visibleSkills = useMemo(() => {
    const q = query.trim().toLowerCase();
    let list = (skills ?? []).filter(s => (q ? t(`insights.skills.${s.key}`)?.toLowerCase()?.includes(q) : true));
  ...
  // Group by tiers for friendlier reading
    ...
    <Container>
      ...
      {err && (
        <div className="mb-4 rounded-xl border border-rose-500/30 bg-rose-900/20 text-rose-200 p-3 flex items-center justify-between">
          <span>{err}</span>
          <button onClick={reload} className="px-3 py-1 rounded-md border border-white/15 bg-white/10 hover:bg-white/15 text-sm">{t("common.retry")}</button>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-12 gap-6">
            ...
            <div className="mt-3 text-xs text-slate-400 flex flex-wrap items-center gap-2">
              ...
              {Object.keys(PLANET_EMOJI).map((p) => (
                <span key={p} className="inline-flex items-center gap-1">
                  <PlanetAvatar name={p} /><span>{p}</span>
                </span>
              ))}
            </div>
          </div>
        ...
        <section className="lg:col-span-7">
          ...
          <div className="mb-3 flex flex-col md:flex-row gap-3 md:items-center">
            ...
            <button
              onClick={() => {
                // copy top view as summary
                const lines = visibleSkills.slice(0, topN === "all" ? 10 : topN).map((s, i) => `${i + 1}. ${t(`insights.skills.${s.key}`)} — ${s.score}/100`);
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                  <div className="text-white font-semibold truncate">{t(`insights.skills.${s.key}`)}</div>
